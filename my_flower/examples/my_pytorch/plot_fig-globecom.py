...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python3 plot_fig-globecom.py <directory1> <directory2> ... <x_axis> <y_axis>")
        sys.exit(1)

    directories = sys.argv[1:-2]
    x_axis = sys.argv[-2]
    y_axis = sys.argv[-1]
    #labels = ["Conventional (timeout35[s])","ATC (timeout35[s])","Conventional (timeout60[s])","ATC (timeout60[s])"]

...
def plot_from_excel(files, x_axis, y_axis, bar_width, figsize, legend_names):
    ...
    colors = ['firebrick','black', 'darkslategray',]
   # hatchs = [ "/" , ".",  "\\" , "|" , "-" , "+" , "x", "o", "O", "*" ]
    ...
    for idx, file in enumerate(files):
       ...
       # hatch = hatchs[idx % len(hatchs)]
        ...
        ax.bar(data[x_axis] + offset, data[y_axis], width=bar_width, label=label_name, color=color)
    ...
    ax.set_ylabel("Computation completion per\n communication round [%]", fontsize=20)
    ax.set_xlabel("Timeout [s]", fontsize=20)

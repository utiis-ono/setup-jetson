...
if len(sys.argv) != 6:
    print("Usage: python3 run_sim.py [1]<round> [2]<time out[s]> [3]<round time[s]> [4]<number of client> [5]<model>")
...
dirname = os.path.join("result",dirpath,"log")
...
command =  f"python3 server.py {dirpath}  {sys.argv[1]} {sys.argv[2]} {sys.argv[3]} 2>&1 | tee {dirname}/result_serverlog.csv"
...
for i in range(iterations):
    ...
    command = f"python3 sim_client2.py {dirpath} {str(i)} {sys.argv[3]} {sys.argv[2]} {sys.argv[5]} 2>&1 | tee {dirname}/result_clientlog-{str(i)}.csv"
    open_terminal_with_command(command)
